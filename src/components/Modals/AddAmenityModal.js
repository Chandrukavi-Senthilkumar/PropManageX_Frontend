...
const AddAmenityModal = ({ isOpen, onClose, propertyID, refreshData }) => {
  ...
  const formik = useFormik({
    ...
    // This ensures the propertyID updates if you click a different card
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        console.error(err);
        alert("Failed to add amenity. Please check your connection.");
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[70]">
      <div className="bg-white rounded-[32px] w-full max-w-md p-8 shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-800">Add Amenity</h2>
        </div>

    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
      <div className="bg-[#FEF8FA] rounded-[40px] w-full max-w-md p-8 relative shadow-2xl">
        ...
        <div className="text-center space-y-4 mt-4">
          ...
          <h2 className="text-xl font-extrabold text-[#1F2937]">Market Value Notice</h2>
          <p className="text-[#6B7280] leading-relaxed">
            This is the market value of this unit. If you need to negotiate the amount, please contact the administrator.
          </p>
          <div className="flex gap-3 pt-6">
            <button onClick={onClose} className="flex-1 py-3 bg-white text-gray-500 rounded-2xl font-semibold shadow-sm">
              Cancel
            </button>
            <button
              onClick={onConfirm}
...
const PropertyDetailsView = ({
  ...
  // ✅ PULL USER PROFILE FROM REDUX
  const { profile } = useSelector((state) => state.admin || {});
  const userRole = profile?.role;

  // ✅ DEFINE PERMISSIONS
  const isBuyer = userRole === 'BuyerAndTenant';
  const isStaff = ['Admin', 'SalesAndLeasingAgent', 'PropertyManager', 'FinanceAnalyst'].includes(userRole);
  ...
  const confirmBooking = async () => {
    try {
      ...
      setUnits((prev) =>
        prev.map((unit) =>
          unit.unitID === bookingModal.unitID ? { ...unit, isBooked: true, status: 'Booked' } : unit
        )
      );
      setBookingModal({ isOpen: false, unitID: null });
      setTimeout(() => showToast?.('Unit booked successfully!', 'success'), 0);
    } catch (err) {
      setBookingModal({ isOpen: false, unitID: null });
      setTimeout(() => showToast?.(err.response?.data?.message || 'Already booked', 'error'), 0);
    }
  };
  ...
  return (
    <div className="space-y-12 pb-20 max-w-7xl mx-auto px-4">
      ...
      <div className="bg-[#FEF8FA] rounded-[32px] shadow-lg p-8">
        ...
        <p className="flex items-center gap-1 text-sm text-[#6B7280] mt-2 mb-6">
          <MapPinIcon className="w-4 h-4" /> {property?.location} • {property?.type}
        </p>
        {property?.imageUrl && (
      ...
      {activeTab === 'units' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {units.map((unit) => {
            ...
            console.log(isBooked, unit.status);
            return (
              <div key={unit.unitID} className="bg-[#FEF8FA] rounded-3xl shadow-lg p-6 space-y-4">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-extrabold">Unit {unit.unitNumber}</h3>
                  <span className={`px-3 py-1 text-xs font-bold rounded-full ${isBooked ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                    {isBooked ? 'Booked' : 'Available'}
                  </span>
                </div>
                <div className="text-xl font-bold text-[#5B3E59] text-center">₹ {unit.basePrice?.toLocaleString('en-IN')}</div>
                <div className="flex gap-3 pt-4">
                  {/* ✅ STAFF: See Edit | BUYER: See Book Now */}
                  {isStaff && (
                    <button onClick={() => onEditUnit(unit)} className="flex-1 py-2 rounded-xl bg-[#F3EEF2] font-semibold text-[#5B3E59]">
                      Edit Unit
                    </button>
                  )}
                  {isBuyer && (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-xl font-semibold transition ${
                        isBooked ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#5B3E59] hover:bg-[#4A3248]'
                      }`}
                    >
          ...
          <table className="w-full text-left">
            <thead className="text-xs uppercase tracking-widest text-[#6B7280]">
              <tr>
                ...
                {isStaff && <th className="py-3">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {amenities.map((amenity) => (
                <tr key={amenity.amenityID} className="hover:bg-white/60">
                  <td className="py-3">{amenity.name}</td>
                  <td className="py-3 text-gray-500">{amenity.description}</td>
                  {isStaff && (
                    <td className="py-3">
                      <button onClick={() => onEditAmenity(amenity)} className="text-[#5B3E59] font-semibold text-sm">
                        Edit
                      </button>
                    </td>
                  )}
        ...
        <div className="bg-[#FEF8FA] rounded-[32px] shadow-lg p-8 space-y-6 overflow-x-auto">
          {isStaff && (
            <div className="flex justify-end">
              <button onClick={onUploadDocument} style={{ color: '#FFFFFF' }} className="flex items-center gap-2 px-5 py-2 rounded-xl bg-[#5B3E59] text-white font-semibold">
      ...
      <BookingConfirmModal isOpen={bookingModal.isOpen} onClose={() => setBookingModal({ isOpen: false, unitID: null })} onConfirm={confirmBooking} />
    </div>
  );
};
...
const TabButton = ({ active, onClick, icon, label }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-3 px-6 py-2.5 rounded-2xl text-sm font-semibold transition ${
